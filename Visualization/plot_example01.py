...
def tmp(x, y):
    return 2.0 * np.exp(-25.0 * (x - 0.6) ** 2 - 25.0 * (y - 0.4) ** 2)
...
# # File names
# exp_idx = '01'
# idxs_nn = ['09_01', '08_10', '00_20', '07_50']
# files = ['non_smooth_udag', 'non_smooth_qdag', 'non_smooth_unn', 'non_smooth_qnn', 'non_smooth_unn_err', 'non_smooth_qnn_err', 'non_smooth_ufem', 'non_smooth_q_fem', 'non_smooth_ufem_err', 'non_smooth_qfem_err']
...
# lamda = '07'
# idxs = ['05_01', '05_10', '05_20', '05_50']
# # Set directory for saving files
# directory = './Figures/non_smooth'
...
#     file_path = f'./TikPINN/output/average02/data2468/result_{idxs_nn[i]}/result50000.txt'
...
#     file_path = f'./TikFEM/output/non_smooth/result_{idxs[i]}/result.txt'
...
# ------------------------------------------------------------------------------------
len_ = 501
idxs = ['08_50', '09_50', '00_50']

q_dags, u_dags, q_nns, u_nns, qnn_errs, unn_errs, q_fems, u_fems, qfem_errs, ufem_errs = [], [], [], [], [], [], [], [], [], []

for i in range(len(idxs)):
    file_path = f'./TikFEM/output/one_peak/result_{idxs[i]}/result.txt'
    results_fem = pd.read_csv(file_path, header=None, sep=',').values
    ...
    q_fems.append(q_fem)
    qfem_errs.append(relative_error(q_fem, q_dag))
    u_fems.append(u_fem)
    ufem_errs.append(relative_error(u_fem, u_dag))
print(np.array(qfem_errs))
print(np.array(ufem_errs))
...
#     q_nn = np.flip(results_nn[:, 2].reshape(len_, len_))
#     u_nn = np.flip(results_nn[:, 3].reshape(len_, len_))

#     q_dags.append(q_dag)
#     u_dags.append(u_dag)
 
#     q_nns.append(q_nn)
#     qnn_errs.append(relative_error(q_nn, q_dag))
#     u_nns.append(u_nn)
#     unn_errs.append(relative_error(u_nn, u_dag))
# print(np.array(qnn_errs))
# print(np.array(unn_errs))