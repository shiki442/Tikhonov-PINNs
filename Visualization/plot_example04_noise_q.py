...
if __name__ == "__main__":
    ...
    for j in range(len(lambdas)):
        ...
        log_err_q = np.log10(err_q[:, j])
        slope_q, _ = np.polyfit(log_noise, log_err_q, 1)
        q_slopses.append(slope_q)

    print(q_slopses, np.mean(q_slopses))
    for i, (y_data, color, marker, label) in enumerate(zip(err_q_list, colors, markers, labels)):
        ax.plot(
            noise_levels,
            y_data,
            marker=marker,
    ...
    y = target_errq(x)
    ax.plot(x, y, linestyle='--', color='grey', label=r'$\log\epsilon_q=0.5\log\delta + C$', linewidth=4)
    ...
    ax.set_title(r'Effect of Noise Level $\delta$ on Prediction Error $\epsilon_q$', fontsize=27)
    # 修改坐标轴刻度格式设置部分
