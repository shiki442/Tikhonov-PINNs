...
for i in range(noise_levels.__len__()):
    ...
    err = np.abs(u_dag - y)
    ax = axes[i]
    ax.plot(x, u_dag, label=r'$u^\dagger$', color='#ef4444', linewidth=2)
    ax.plot(x, y, label=r'$u_{pinn}$', color='#000000', linewidth=2, linestyle=':')
    ...
    ax2 = ax.twinx()
    ax2.plot(x, smooth_data(err, window_size=15), label=r'$|u^\dagger - u_{pinn}|$', color='#7f8c8d', linewidth=2, linestyle='--')
    ax2.set_ylabel(r'$|u^\dagger - u_{pinn}|$', fontsize=20)
    ax2.set_ylim(0, 0.02)
