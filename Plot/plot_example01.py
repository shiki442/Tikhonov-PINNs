...
def plot_subplots(values, str_params, title=None, filename=None, vmin=None, vmax=None):
    ...
    gs = gridspec.GridSpec(1, 7, width_ratios=[1, 0.3, 1, 1, 1, 1, 0.05])  # 最后一列留给 colorbar
    imgs = []
...
if __name__ == "__main__":
    ...
    # file_path = f'./TikPINN_fullobs/output/loss/result50000.txt'
    ...
    # plt.imshow(q_nn, cmap='coolwarm', aspect='auto')
    # plt.colorbar()
    # plt.show()
    ...
    for i in range(len(str_params)):
        file_path = f'./TikPINN_fullobs/output/one_peak/result_{str_params[i]}/result50000.txt'
