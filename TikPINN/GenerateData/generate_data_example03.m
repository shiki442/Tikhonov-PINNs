...
tmp = @(x,y)exp(-9.0*(x-0.5).^2-9.0*(y-0.5).^2);
q_dagger = @(x,y)1.0+0.5*(x>=y).*tmp(x,y);
...
for delta = [0.01,0.10,0.20]
    data_mat = generate_data_2d(q_dagger,u_dagger,grad_u_x,grad_u_y,laplace_u,delta);
    file_name = ['../data/example03data',num2str(100*delta,'%02d'),'.txt'];
