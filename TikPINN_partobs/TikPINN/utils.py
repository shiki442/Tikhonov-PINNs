...
def output_path(config, noise_str: str):
    lamb = config["loss_params"]["lamb"]
    if lamb == 0.0:
        config["train_params"]["results_path"] = f"result_00_{noise_str}"
    else:
        lamb = int(-np.log10(lamb))
        config["train_params"]["results_path"] = f"result_0{lamb}_{noise_str}"
    os.makedirs(config["train_params"]["results_path"], exist_ok=True)
    return config
